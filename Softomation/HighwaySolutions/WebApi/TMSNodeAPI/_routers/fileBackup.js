...
router.post('/FileBackUpDetailInsertUpdate', FileBackUpDetailInsertUpdate);
router.get('/FileBackUpDetailGetAll', FileBackUpDetailGetAll);
router.get('/FileBackUpDetailGetById', FileBackUpDetailGetById);
...
async function FileBackUpDetailInsertUpdate(req, res, next) {
    try {
        const pool = await database.getPool();
        const currentDateTime = new Date();
        result = await pool.request().input('FileBackUpDetailId', sql.SmallInt, req.body.DBBackUpDetailId)
            ...
            .input('FileName', sql.VarChar(50), req.body.FileName)
            .input('FileTypeId', sql.SmallInt, req.body.FileTypeId)
            ...
            .execute('USP_FileBackUpDetailInsertUpdate');
        let out = constants.ResponseMessageList(result.recordset, null);
        res.status(200).json(out)
    } catch (error) {
        errorlogMessage(error, 'FileBackUpDetailInsertUpdate');
        let out = constants.ResponseMessage(error.message, null);
        res.status(400).json(out);
    }
}

async function FileBackUpDetailGetAll(req, res, next) {
    try {
        const pool = await database.getPool();
        result = await pool.request().execute('USP_FileBackUpDetailGetAll');
        
        let out = constants.ResponseMessage("success", result.recordsets);
        res.status(200).json(out)
    } catch (error) {
        errorlogMessage(error, 'FileBackUpDetailGetAll');
        let out = constants.ResponseMessage(error.message, null);
        res.status(400).json(out);
    }
}

async function FileBackUpDetailGetById(req, res, next) {
    try {
        const FileBackUpDetailId = req.query.FileBackUpDetailId | 0;
        const pool = await database.getPool();
        result = await pool.request().input('FileBackUpDetailId', sql.SmallInt, FileBackUpDetailId)
            .execute('USP_FileBackUpDetailGetById');
        if (result.recordset == []) {
            let out = constants.ResponseMessage("No data found", null);
            res.status(200).json(out);
        }
        ...
        errorlogMessage(error, 'FileBackUpDetailGetById');
        let out = constants.ResponseMessage(error.message, null);
        res.status(400).json(out);
    }
}
