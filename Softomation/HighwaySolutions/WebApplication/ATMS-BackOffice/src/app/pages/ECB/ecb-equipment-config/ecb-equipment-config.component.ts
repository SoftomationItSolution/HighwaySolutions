...
@Component({
  selector: 'app-ecb-equipment-config',
  templateUrl: './ecb-equipment-config.component.html',
  styleUrls: ['./ecb-equipment-config.component.css']
})
export class ECBEquipmentConfigComponent implements OnInit {
  availableData: any = [];
  selection: any = null;
  fillData: any = [];
  FillFinalData: any = [];
  ...
  EquipmentType() {
    this.dbService.EquipmentTypeGetActive().subscribe(
      data => {
        ...
        dataType = dataType.filter((e: { EquipmentTypeId: any; }) => e.EquipmentTypeId == 28 || e.EquipmentTypeId == 2 || e.EquipmentTypeId == 12);
  ...
  onNodeDrop() {
    if (this.fillData.length == 0) {
      if (this.selection.EquipmentTypeId == 12) {
  ...
  ResetNodes() {
    ...
    var child0 = []
    var IPList = [];
    var p0 = this.fillData.filter((e: { EquipmentTypeId: any; }) => e.EquipmentTypeId == 12);
    if (p0.length > 0) {
      ...
      let mainObj = {
        key: "IPPhone",
        label: "Control Room",
        ChainageName: p0[0].ChainageName,
        IPPhone: IPList,
      }
      this.FillFinalData.push(mainObj)
    }
    var p1 = this.fillData.filter((e: { EquipmentTypeId: any; }) => e.EquipmentTypeId == 28);
    if (p1.length > 0) {
      for (let j = 0; j < p1.length; j++) {
        child0.push(p1[j])
        let element1 = p1[j]
        var ECBList = [];
        var p2 = this.fillData.filter((e: { EquipmentTypeId: any, ChainageName: any; }) => e.EquipmentTypeId == 2 && e.ChainageName == element1.ChainageName);
        if (p2.length > 0) {
  ...
  masterExCol(event: any) {
    jQuery("#FillDataList .nested").toggle("nested-active")
    if(event.target.classList.contains('parentNode-down'))
      jQuery("#FillDataList span").removeClass("parentNode-down")
    else
      jQuery("#FillDataList span").addClass("parentNode-down")
    event.target.classList.toggle("parentNode-down")
  }
  ...
  SaveDetails() {
    if (this.FillFinalData.length == 0) {
      ...
      var p0 = this.FillFinalData.filter((e: { key: any; }) => e.key == "IPPhone");
      var p1 = this.FillFinalData.filter((e: { key: any; }) => e.key != "IPPhone");
      for (let k = 0; k < p0.length; k++) {
        let element = p0[k]
        for (let j = 0; j < element.IPPhone.length; j++) {
          let Lelement = element.IPPhone[j];
          let obj = {
            SystemId: this.SystemId,
