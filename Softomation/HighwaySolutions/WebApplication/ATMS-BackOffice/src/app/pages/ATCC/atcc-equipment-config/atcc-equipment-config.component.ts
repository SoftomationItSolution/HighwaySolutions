...
@Component({
  selector: 'app-atcc-equipment-config',
  templateUrl: './atcc-equipment-config.component.html',
  styleUrls: ['./atcc-equipment-config.component.css']
})
export class ATCCEquipmentConfigComponent implements OnInit {
  availableData: any = [];
  selection: any = null;
  fillData: any = [];
  FillFinalData: any = [];
  ...
  EquipmentType() {
    this.dbService.EquipmentTypeGetActive().subscribe(
      data => {
        ...
        dataType = dataType.filter((e: { EquipmentTypeId: any; }) => e.EquipmentTypeId == 3 || e.EquipmentTypeId == 25 || e.EquipmentTypeId == 27 || e.EquipmentTypeId == 28 ||  e.EquipmentTypeId == 32);
  ...
  ResetNodes() {
    ...
    if (p0.length > 0) {
      for (let j = 0; j < p0.length; j++) {
        ...
        var CameraList = [];
        var p2 = this.fillData.filter((e: { EquipmentTypeId: any, ChainageName: any; }) => e.EquipmentTypeId == 32 && e.ChainageName == element1.ChainageName);
        if (p2.length > 0) {
