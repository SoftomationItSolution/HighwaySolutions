...
@Component({
  selector: 'app-vms-equipment-config',
  templateUrl: './vms-equipment-config.component.html',
  styleUrls: ['./vms-equipment-config.component.css']
})
export class VMSEquipmentConfigComponent implements OnInit {
  availableData: any = [];
  selection: any = null;
  fillData: any = [];
  FillFinalData: any = [];
  ...
  SystemId = 0;
  PositionList = [{ DataId: 1, DataName: "Entry" }, { DataId: 2, DataName: "Exit" }, { DataId: 3, DataName: "Main Carriageway" }]
  ...
  EquipmentType() {
    this.dbService.EquipmentTypeGetActive().subscribe(
      data => {
        ...
        dataType = dataType.filter((e: { EquipmentTypeId: any; }) => e.EquipmentTypeId == 27 || e.EquipmentTypeId == 28 ||  e.EquipmentTypeId == 8);
  ...
  ResetNodes() {
    ...
    if (p0.length > 0) {
      for (let j = 0; j < p0.length; j++) {
        ...
        let element1 = p0[j]
        var VMSList = [];
        var p2 = this.fillData.filter((e: { EquipmentTypeId: any, ChainageName: any; }) => e.EquipmentTypeId == 8 && e.ChainageName == element1.ChainageName);
        if (p2.length > 0) {
