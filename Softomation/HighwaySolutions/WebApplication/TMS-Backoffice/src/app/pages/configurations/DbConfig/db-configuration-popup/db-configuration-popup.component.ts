...
export class DBConfigurationPopupComponent implements OnInit {
  ...
  error = errorMessages;
  FileBackUpDetailId: number = 0;
  LogedUserId: number = 0;
  FileFileTypeId: number = 0;
  DetailData: any=null;
  submitted = false;
  ErrorData: any;
  hide = true;
  ...
  constructor(private spinner: NgxSpinnerService, @Inject(MAT_DIALOG_DATA) parentData: any, public Dialogref: MatDialogRef<DBConfigurationPopupComponent>,
    ...
    this.FileBackUpDetailId = parentData.FileBackUpDetailId;
    this.FileTypeList=parentData.FileTypeMaster;
  ...
  DetailsbyId() {
    this.spinner.show();
    this.dbService.FileBackUpDetailGetById(this.FileBackUpDetailId).subscribe(
      data => {
        this.spinner.hide();
  ...
  fillDataByDB() {
    this.DataDetailsForm.controls['FileTypeId'].setValue(this.DetailData.FileTypeId);
    ...
    this.DataDetailsForm.controls['FileName'].setValue(this.DetailData.FileName);
  ...
  SaveDetails() {
    ...
    const Obj = {
      DBBackUpDetailId: this.FileBackUpDetailId,
      FileTypeId: this.DataDetailsForm.value.FileTypeId,
      HostName: this.DataDetailsForm.value.HostName,
      HostPort: this.DataDetailsForm.value.HostPort,
      FileName: this.DataDetailsForm.value.FileName,
    ...
    this.spinner.show();
    this.dbService.FileBackUpDetailInsertUpdate(Obj).subscribe(
      data => {
        this.spinner.hide();
