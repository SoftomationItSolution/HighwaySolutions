...
export class TransactionalDataComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  DataView: Number = 0;
  DataUpdate: Number = 0;
  TypeList = [{ Id: 0, Name: "All Transactions" }, { Id: 1, Name: "Towed Transaction" }, { Id: 2, Name: "Simulation Transaction" },
  { Id: 3, Name: "Override Transaction" }, { Id: 4, Name: "Cancel Transaction" }, { Id: 5, Name: "Fleet Transaction" },
  { Id: 6, Name: "Overweight" }, { Id: 7, Name: "AVC Mismatch" }]
  PlazaDataList: any;
  LaneData: any
  LaneList: any = [];
  UserDetails: any;
  ...
  ngOnInit(): void {
    this.FilterDetailsForm = new FormGroup({
      ...
      UserFilterList: new FormControl('', []),
  ...
  GetShiftData() {
    this.subscription = this.dbService.GetShiftTimining().subscribe(
      data => {
  ...
  GetLaneUsersData() {
    this.subscription = this.dbService.UserConfigurationGetByUserType(4).subscribe(
      data => {
  ...
  GetLaneData() {
    this.subscription = this.dbService.LaneGetActive().subscribe(
      data => {
  ...
  GetTransactionTypeData() {
    this.subscription = this.dbService.TransactionTypeGetActive().subscribe(
      data => {
        this.TransactionTypeData = data.ResponseData;
        this.GetPlaza();
      },
      (error) => {
        this.spinner.hide();
  ...
  GetPlaza() {
    this.subscription = this.dbService.PlazaGetActive().subscribe(
      data => {
        this.PlazaDataList = data.ResponseData;
        this.GetLane();
      },
      (error) => {
        this.spinner.hide();
  ...
  GetLane() {
    this.subscription = this.dbService.LaneGetActive().subscribe(
      data => {
        this.LaneDataList = data.ResponseData;
        this.GetClassData();
      },
      (error) => {
        this.spinner.hide();
  ...
  GetClassData() {
    this.subscription = this.dbService.SystemVehicleClassGetActive().subscribe(
      data => {
  ...
  PlazaChange() {
    ...
    for (let index = 0; index < pl.length; index++) {
      var d1 = this.LaneData.filter((e: { PlazaId: any; }) => e.PlazaId == pl[index]);
  ...
  VehicleClassChange() {
    ...
    for (let index = 0; index < vc.length; index++) {
      var d1 = this.ClassData.filter((e: { SystemVehicleClassId: any; }) => e.SystemVehicleClassId == vc[index]);
      let sub = d1[0].SystemSubClassList;
      for (let j = 0; j < sub.length; j++) {
        this.SubClassData.push(sub[j])
      }
  ...
  FilterAllData() {
    ...
    let UserFilterList = "0"
    if (this.FilterDetailsForm.value.UserFilterList != null && this.FilterDetailsForm.value.UserFilterList != '') {
      let crData = this.FilterDetailsForm.value.UserFilterList.toString();
      if (crData.split(',').length != this.LaneUserData.length) {
        UserFilterList = this.FilterDetailsForm.value.UserFilterList.toString();
      }
    }
    ...
    if (this.FilterDetailsForm.value.VehicleSubClassFilterList != null && this.FilterDetailsForm.value.VehicleSubClassFilterList != '') {
      ...
      if (crData.split(',').length != this.SubClassData.length) {
