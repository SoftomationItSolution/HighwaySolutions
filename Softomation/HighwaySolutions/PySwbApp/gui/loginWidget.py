...
class LoginWidget(QWidget):
    ...
    def __init__(self,default_directory,plaza_config,logger:CustomLogger,system_ip):
        super().__init__()
        self.logger=logger
    ...
    def authenticate(self):
        ...
        try:
            api_url=apipath+"ValidateLaneUser"
            ...
            if len(data["Message"])>0:
                if data["Message"][0]["AlertMessage"]=="success":
                    ...
                    x={"UserId":user_data["UserData"]["UserId"],"LocalAddress":self.system_ip}
                    self.login_activity(x)
                    self.successful_login.emit(user_data)
                else:
