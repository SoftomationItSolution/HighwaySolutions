...
class MSSQLConnectionManager:
    ...
    def execute_query(self, query):
        ...
        with connection.cursor() as cursor:
            try:
                ...
                connection.commit()
                # Only fetch results if the query is expected to return data
                if cursor.description:  # Check if there are columns to fetch
                    rows = cursor.fetchall()
                    if rows:
                        columns = [column[0] for column in cursor.description]
                        df = pd.DataFrame.from_records(rows, columns=columns)
                        return df
                return None  # If no result set, return None
            except Exception as e:
    ...
    def execute_stored_procedure(self, procedure_name, params=None):
        """Execute a stored procedure, ensuring one query per connection."""
        connection = self._connect()
        with connection.cursor() as cursor:
            try:
                if params:
                ...
                # Only attempt to fetch if the procedure is expected to return data
                if cursor.description:  # Check if there are columns to fetch
                    columns = [column[0] for column in cursor.description]
                    rows = cursor.fetchall()
                    if rows:
    ...
    def execute_stored_procedure_single(self, procedure_name, params=None):
        ...
        with connection.cursor() as cursor:
            try:
                ...
                return None  # If no result row, return None
            except Exception as e:
                self.logger.logError(f"Error executing stored procedure: {e}")
                raise e
