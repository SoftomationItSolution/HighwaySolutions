...
class RequestExceptionList(threading.Thread):
    ...
    def generate_xml_dif(self):
            try:
                data = self.dbConnectionObj.execute_stored_procedure("USP_ICDRequestExceptionDetailGetLastRequest")
                if data:
                    for row in data:
                        RequestCode=str(row["RequestCode"])
                        if RequestCode!="00":
                            MessageId=Utilities.generate_message_id(str(self.plaza_details["PlazaZoneId"]))
                            req_time=Utilities.get_date_time()['CurrentDateTime_s']
                            apiname=f'ReqQueryException_{RequestCode}_'
                            file_name=Utilities.generate_file_path(self.default_directory,None,apiname,MessageId)
                            namespace = "http://npci.org/etc/schema/"
                            register_namespace("etc", namespace)
                            ...
                            ExceptionList = SubElement(txn, "ExceptionList")
                            Exception = SubElement(ExceptionList, "Exception")
                            Exception.set("excCode",RequestCode)
                            Exception.set("lastFetchTime",Utilities.icd_datetime_format(row["LastUpdatedDateTime"]))
                            xml_string = Utilities.prettify_xml(root)
                            signed_xml = XmlSigner.sign_xml_file(xml_string, file_name,self.private_key, self.certificate)
                            if signed_xml:    
                                try:
                                    ...
                                    if response.ok:
                                        self.update_request(RequestCode,req_time,True)
                                    else:
                                        self.update_request(RequestCode,req_time,False)
                                        self.logger.logError(f"Error: txn {MessageId} failed with StatusCode {response.status_code}, StatusDescription: {response.text}")
                                except Exception as e:
    ...
    def update_request(self,RequestCode,req_time,statusId):
        try:
            formatted_date = Utilities.icd_to_mssql_datetime_format(req_time)
            self.dbConnectionObj.execute_stored_procedure("USP_ICDRequestExceptionDetailUpdateRequest",params=[RequestCode,formatted_date, statusId])
        except Exception as e:
            self.logger.logError(f"Error send update_request: {e}")