...
class RequestCheckStatus(threading.Thread):
    ...
    def generate_xml(self, data):
        try:
            ...
            if signed_xml:    
                try:
                    response=SendRequest.upload_request(signed_xml, F"{self.icd_api_detail['BaseUrl']}{self.icd_api_detail['CheckTransactionStatusRequestURL']}",certificate_validation=1)
                    if response.ok:
                        if response.text!='':
