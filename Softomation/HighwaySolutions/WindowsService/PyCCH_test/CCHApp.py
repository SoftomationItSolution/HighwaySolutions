...
from utils.cch_processing import CCHProcessing
from utils.constants import Utilities
from utils.log_master import CustomLogger

class CCHAppv1:
    def __init__(self):
        self.PID_FILE = "ICD_testv1.pid"
        self.stop_event = threading.Event()
        self.default_directory = self.check_default_dir()
        self.script_dir=os.path.dirname(os.path.abspath(__file__))
        self.logger = CustomLogger(self.default_directory, 'icd_app_test')
    ...
    def main(self):
        try:
            ...
            db_manager = AsyncMSSQLConnectionManager(self.default_directory, server=db_json_data['server'],database=db_json_data['database'], username=db_json_data['user'], password=db_json_data['password'])
            ...
            self.bg_handler = CCHProcessing(self.default_directory, dbConnectionObj,db_manager, db_json_data,self.script_dir)
