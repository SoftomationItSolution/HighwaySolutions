...
import os
import requests
from utils.constants import JsonDateFormat, get_absolute_file_path
from utils.crypt import encrypt_aes_256_cbc
from datetime import date, timedelta

class DataImporter:
    def __init__(self, config_manager,db,logger,default_plaza_Id,default_lane_ip):
    ...
    def import_data_list(self, endpoint, params, procedure_name):
        try:
            ...
            if response.status_code == 200:
                ...
                for d in data:
                    result_data = self.db.execute_procedure(procedure_name, params(d))
    ...
    def import_fare_data(self, endpoint):
        try:
            ...
            if response.status_code == 200:
                ...
                resultData=self.db.execute_procedure('USP_TollFareMasterInsertUpdate', params)
                if resultData[0]['AlertMessage'] != 'Cannot update toll fare. EffectedFrom must be greater than the current date;':
                    for d in data['TollFareConfigurations']:
                        ...
                        resultData1=self.db.execute_procedure('USP_TollFareConfigurationInsertUpdate', params)
    ...
    def equipment_type_Import(self):
        ...
        params = lambda d: [d['EquipmentTypeId'],d['EquipmentTypeName'],d['EquipmentCategoryTypeId'],
    ...
    def system_vehicleclass_Import(self):
        ...
        params = lambda d: [d['SystemVehicleClassId'],d['SystemVehicleClassName'],d['SystemVehicleClassDescription'],
    ...
    def users_Import(self):
        ...
        params = lambda d: [d['UserId'],d['LoginId'],encrypt_aes_256_cbc(d['LoginPasswordPlan']),d['FirstName'],
                            d['LastName'],d['EmailId'],d['MobileNumber'],d['AccountExpiredDate'],d['PlazaId'],
                            d['UserTypeId'],d['RoleId'],
                            d['DataStatus'], d['CreatedDate'], d['CreatedBy'], d['ModifiedDate'], d['ModifiedBy']]
    ...
    def plaza_import(self):
        ...
        params = lambda d: [d['PlazaId'],d['SystemIntegratorId'],d['PlazaName'],d['PlazaTypeId'],d['PlazaServerIpAddress'],
                            d['ChainageNumber'],d['Latitude'],d['Longitude'],d['PlazaZoneId'],
                            d['DataStatus'], d['CreatedDate'], d['CreatedBy'], d['ModifiedDate'], d['ModifiedBy']]
