        ...
        try:
            if not self.rfid_thread:
                if equipment["ManufacturerName"]=="Mantra":
                    ...
                    self.rfid_thread = IdTechRfidReader(self,self.default_directory, equipment, 'lane_BG_rfid')
                    self.rfid_thread.daemon=True
                    self.rfid_thread.start()
        except Exception as e:
        ...
        try:
            if self.equipment_detail is not None:
                for equipment in self.equipment_detail:
                    if equipment["EquipmentTypeId"]==3:
                        self.presence_loop_required=True
                    elif equipment["EquipmentTypeId"]==4:
