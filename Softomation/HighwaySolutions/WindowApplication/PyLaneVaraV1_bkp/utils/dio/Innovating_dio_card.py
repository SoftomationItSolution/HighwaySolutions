...
class InnovatingDIOClient(threading.Thread):
    ...
    def process_data(self,data):
        try:
            if len(data)>0:
                formatted_data = [data.strip() for d in data if d]
                for cmd in formatted_data:
                    cmd_upper = cmd.upper()
                    if 'INPUT4LOW' in cmd_upper:
                        self.handel_presence_loop(False)
                    elif 'INPUT4HIGH' in cmd_upper:
                        self.handel_presence_loop(True)
                    elif 'INPUT5LOW' in cmd_upper:
                        self.handel_exit_loop(False)
                    elif 'INPUT5HIGH' in cmd_upper:
    ...
    def serial_conn(self):
        try:
            ...
            while self.is_running:
                ...
                #received_data=self.client_socket.read_until()
