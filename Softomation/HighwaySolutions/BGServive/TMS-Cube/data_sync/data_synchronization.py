...
class DataSynchronization(threading.Thread):
    ...
    def data_impoter(self,dbConnectionObj):
        try:
            self.data_importer = DataImporter(self.default_directory, dbConnectionObj,self.default_plaza_Id,self.default_lane_ip)
        except Exception as e:
    ...
    def start_data_uploading_threads(self):
        ...
        threading.Thread(target=self.lane_lpic_uploading).start()
        threading.Thread(target=self.lane_ic_uploading).start()
    ...
    def lane_lpic_uploading(self):
        while self.data_upload_running:
            try:
                result_data =LaneManager.lane_lpic_media_pending(self.dbConnectionObj)
                for s in result_data:
                    if self.data_upload_running==False:
                        break
                    try:
                        ...
                        LaneManager.lane_lpic_media_marked_transfer(self.dbConnectionObj,s)
                    except Exception as e:
                        self.logger.logError(f"Exception lane_lpic_uploading child: {str(e)}")
                    finally:
                        if self.data_upload_running==False:
                            break
                        time.sleep(self.timeout)
            except Exception as e:
                self.logger.logError(f"Exception lane_lpic_uploading: {str(e)}")
            finally:
                if self.data_upload_running==False:
                    break
                time.sleep(self.timeout)

    def lane_ic_uploading(self):
        while self.data_upload_running:
            try:
                result_data =LaneManager.lane_ic_media_pending(self.dbConnectionObj)
                for s in result_data:
                    if self.data_upload_running==False:
                        break
                    try:
                        ...
                        if s["TransactionVideo"] is not None and s["TransactionVideo"]!='':
                            ...
                            if Utilities.check_file_exists(file_path_dir):
                                ...
                                Utilities.upload_file_ssh(file_path_dir, uploadPath, self.plaza_config["FtpAddress"], self.plaza_config["FtpUser"], self.plaza_config["FtpPassword"])
                        LaneManager.lane_ic_media_marked_transfer(self.dbConnectionObj,s)
                    except Exception as e:
                        self.logger.logError(f"Exception lane_ic_uploading child: {str(e)}")
                    finally:
                        if self.data_upload_running==False:
                            break
                        time.sleep(self.timeout)
            except Exception as e:
                self.logger.logError(f"Exception lane_ic_uploading: {str(e)}")
            finally:
                if self.data_upload_running==False:
                    break
                time.sleep(self.timeout)
