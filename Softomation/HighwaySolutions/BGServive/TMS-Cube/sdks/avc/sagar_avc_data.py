...
class SagarAVCDataClient(threading.Thread):
    def __init__(self,_handler,default_directory,dbConnectionObj,LaneId,avc_detail,vehicle_class,log_file_name,timeout=0.100):
            ...
            if len(avc_data)==8:
                ...
                if AvcSensorClassId>0 or AvcSensorClassId<8:
                    ...
                    transactionInfo = {
                        ...
                        'AvcClassId': self.get_fasttag_class(AvcSensorClassId),
                        'AxleCount': AxleCount,
    ...
    def get_fasttag_class(self,classid):
        try:
            classid=int(classid)
            matched_item = next((item for item in self.vehicle_class if int(item['AvcVehicleClassId']) == classid), None)
            if matched_item:
                return int(matched_item['SystemVehicleClassId'])
            else:
                return classid
        except Exception as e:
