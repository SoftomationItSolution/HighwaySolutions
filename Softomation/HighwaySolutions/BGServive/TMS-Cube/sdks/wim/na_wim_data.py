...
class NAWinDataClient(threading.Thread):
    ...
    def process_total_weight(self, total_weight_str):
        try:
            ...
            self.logger.logError(f"Exception process_total_weight: {str(e)}")

    def process_transaction_id(self, transaction_id_str):
        try:
            self.transactionId = transaction_id_str
        except Exception as e:
            self.logger.logError(f"Exception process_transaction_id: {str(e)}")
    ...
    def run(self):
        while not self.is_stopped:
            try:
                ...
                #self.logger.logError(f"Exception wim_data_run: {str(e)}")
                pass
