...
class LaneBGProcess(threading.Thread):
    def __init__(self, default_directory, dbConnectionObj, script_dir, system_ip,ic_timemout):
        ...
        self.load_cam_config(default_directory)
    ...
    def load_cam_config(self,default_directory):
        try:
            dirlist=[default_directory, 'MasterConfig', 'camConfig.json']
            cam_config_path = Utilities.get_final_path(dirlist)
            self.logger.logInfo(f"cam_config_path : {cam_config_path}")
            self.cam_config=Utilities.read_json_file(cam_config_path)
        except Exception as e:
            self.logger.logError(f"Exception load_cam_config: {str(e)}")
            ...
            if self.web_socket_thread and data is not None:
                ...
                if output is not None:
                    if self.web_socket_thread:
                        self.web_socket_thread.send(output)
        except Exception as e:
    ...
    def start_ws_thread(self):
        try:
            if self.web_socket_thread is None:
                self.web_socket_thread=wsSocketClient(self.default_directory)
    ...
    def stop_ws_thread(self):
        try:
            if self.web_socket_thread is not None:
                self.web_socket_thread.stop()
                self.web_socket_thread.join(timeout=2)  # wait for thread to exit
                self.web_socket_thread = None
                self.logger.logInfo("WebSocket thread stopped")
        except Exception as e:
    ...
    def start_lpic_thread(self,equipment):
        try:
            if self.lpic_thread is None:
                self.lpic_thread=IpCameraHandler(self,self.default_directory,"lpic","lane_BG_camera",equipment,self.cam_config)
    ...
    def start_ic_thread(self,equipment):
        try:
            if self.ic_thread is None:
                self.ic_thread=IpCameraHandler(self,self.default_directory,"ic","lane_BG_camera",equipment,self.cam_config)
    ...
    def screenshort_lpic(self,lane_Transaction_img):
        try:
            if self.lpic_thread:
                ...
                return self.lpic_thread.get_screenshot(lane_Transaction_img_path)
            else:
                return None    
        except Exception as e:
    ...
    def start_ic_record(self,transactionInfo):
        try:
            if self.ic_thread is not None:
                ...
                self.ic_thread.start_recording(file_name,"end")
    ...
    def update_media_status(self,camera_name,filename):
        try:

            LaneTransactionId=re.sub(r'(_ic\.mp4|_ic\.jpeg|_lpic\.jpeg|_lpic\.mp4)$', '', filename)
            data={"LaneTransactionId":LaneTransactionId,"CameraName":camera_name}
            LaneManager.lane_media_marked_pending(self.dbConnectionObj,data)
        except Exception as e:
            self.logger.logError(f"Exception update_media_status: {str(e)}")
    ...
    def update_rfid_data(self,data):
        try:
            if self.system_loging_status:
                current_epc:str=data["EPC"]
                if current_epc != self.last_epc:
                    if current_epc.startswith(("3400", "3000")):
                        current_epc = current_epc[4:]    
                    if self.system_transcation_status:
                        ...
                        self.publish_data("tag_status",tag_status)
    ...
    def lane_trans_start(self, transactionInfo):
        ...
        try:
            if self.system_transcation_status==False:
                ...
                if result:
                    running_Transaction["TransactionBackImage"]=f"{str(lane_Transaction_Id)}_ic.jpg"
                    running_Transaction["TransactionVideo"]=f"{str(lane_Transaction_Id)}_ic.mp4"
                self.system_transcation_status=False
                result=True
            else:
            ...
            self.logger.logError(f"Exception lane_trans_start: {repr(e)}\n{traceback.format_exc()}")
            result=False
        finally:
    ...
    def lane_fleet_start(self,transactionInfo):
        ...
        try:
            if self.system_transcation_status==False:
                ...
                self.logger.logInfo(f"trans already in progress lane_fleet_start")
                result=False
        except Exception as e:
