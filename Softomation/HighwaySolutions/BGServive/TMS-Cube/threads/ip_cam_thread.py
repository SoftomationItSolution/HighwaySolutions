...
class IpCameraHandler(threading.Thread):
        ...
        while not self.is_stopped:
            try:
                ...
                while self.stream_online and not self.is_stopped:
                    ...
                    # Distribute frame to all active recording queues
    ...
    # ---------------------------------------------------------
    def start_recording(self, output_file, screenshot_mode=None):
        if not self.stream_online:
            self.logger.logInfo(f"{self.camera_name} Camera offline in start_recording")
            return False
        duration = self.record_timeout  # always use camera config
        ...
        # create per-recording queue
        frame_queue = deque(maxlen=self.frame_rate * duration * 2)
        job = {
            "queue": frame_queue,
            "writer": writer,
    ...
    def update_status(self,file_path):
        try:
            ...
            self.handler.update_media_status(self.camera_name,file_name)
        except Exception as e:
