...
namespace Softomation.ATMSSystemLibrary.DL
{
    internal class IncidentSourceDL
    {
        ...
        internal static List<IncidentSourceIL> GetActive()
        {
            ...
            {
                srcList = GetAll();
                return srcList.FindAll(n => n.DataStatus == (short)Constants.DataStatusType.Active);
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        private static IncidentSourceIL CreateObjectFromDataRow(DataRow dr)
        {
            ...
            src.DataStatusName = Enum.GetName(typeof(Constants.DataStatusType), (Constants.DataStatusType)src.DataStatus);
            return src;
        }
        #endregion
    }
