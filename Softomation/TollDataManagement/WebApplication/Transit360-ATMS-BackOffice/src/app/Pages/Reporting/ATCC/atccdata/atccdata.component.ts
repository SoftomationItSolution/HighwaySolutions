...
@Component({
  ...
  templateUrl: './atccdata.component.html',
  styleUrls: ['./atccdata.component.css']
})
export class ATCCDataComponent implements OnInit {
  DataAdd = 1;
  DataUpdate = 1;
  DataView = 1;
  LogedRoleId;
  ...
  GetDeviceData() {
    this.spinner.show();
    this.dbService.ATCCGetLatest().subscribe(
      data => {
        this.spinner.hide();
  ...
  GetPermissionData() {
    ...
    ViewImage(imageName) {
      ...
      let win = window.open(this.ApiCallUrl + '\ATCC\\image\\' + imageName, '', 'width=600,height=600');
      win.focus();
    }
