...
class TagRequestApi:
    ...
    async def by_bank(self, item):
        try:
            ...
            for entry in resp_code:
                entry['ISSUEDATE'] = datetime.strptime(entry['ISSUEDATE'], '%Y-%m-%d')
            resp_code.sort(key=lambda x: x['ISSUEDATE'])
    ...
    def run(self):
        try:
            ...
            config = Config(self.app, host="0.0.0.0", port=5004, workers=workers)
