...
router.post('/ShiftStatusClose', ShiftStatusClose);
...
async function ShiftStatusClose(req, res, next) {
    try {
        const pool = await database.getPool();
        result = await pool.request().input('ShiftId', sql.SmallInt, req.body.ShiftId).input('ShiftDate', sql.Date, req.body.ShiftDate)
        .execute('USP_ShiftClose');
        let out = constants.ResponseMessageList(result.recordset, null);
        res.status(200).json(out)
    } catch (error) {
        errorlogMessage(error, 'ShiftStatusClose');
        let out = constants.ResponseMessage(error.message, null);
        res.status(400).json(out);
    }
}
