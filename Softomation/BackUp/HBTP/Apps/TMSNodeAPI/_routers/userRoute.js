...
router.post('/UserLoginActivity', UserLoginActivity);
        ...
        if (result == null) {
            ...
            if (result.recordset != []) {
                ...
                if (req.body.LoginPassword == crypto.decrypt(userData.LoginPassword)) {
                    const ipAddress = constants.getClientIp(req); // Public IP
                    let localIpAddress = req.connection.localAddress || "Unknown";
                    const resultMessage = await executeLoginActivityLog(userData.UserId, ipAddress, localIpAddress);
                    if (resultMessage === "Login successful") {
                        let out = constants.ResponseMessage("success", userData);
                        res.status(200).json(out)
                    }else {
                        let out = constants.ResponseMessage(resultMessage, null);
                        res.status(200).json(out)
                    }
                }
                else {
...
async function UserLoginActivity(req, res, next) {
    try {
        const ipAddress = constants.getClientIp(req); // Public IP
        let localIpAddress = req.connection.localAddress || "Unknown";
        if (localIpAddress == "127.0.0.1" || localIpAddress == "Unknown") {
            localIpAddress = req.body.LocalAddress
        }
        const resultMessage = await executeLoginActivityLog(req.body.UserId, ipAddress, localIpAddress);
        if (resultMessage === "Login successful") {
            res.status(200).json({ status: true, message: resultMessage });
        } else {
            res.status(200).json({ status: false, message: resultMessage });
        }
    } catch (error) {
        errorlogMessage(error, 'UserLoginActivity');
        let out = constants.ResponseMessage(error.message, null);
        res.status(400).json(out);
    }
}
...
async function UserLogOut(req, res, next) {
    try {
        ...
        const pool = await database.getPool();
        result = await pool.request().input('UserId', sql.BigInt, req.body.UserId)
        .output("ResultMessage", sql.NVarChar(100))
        .execute('USP_UserLogout');
        const resultMessage = result.output.ResultMessage;
        res.status(200).json({ message: resultMessage });
    } catch (error) {
        errorlogMessage(error, 'UserLogOut');
        let out = constants.ResponseMessage(error.message, null);
        res.status(400).json(out)
    }
}

const executeLoginActivityLog = async (userId, ipAddress, localIpAddress) => {
    try {
        const pool = await database.getPool();
        const result = await pool
            ...
            .input("LocalIPAddress", sql.NVarChar(50), localIpAddress)
            .output("ResultMessage", sql.NVarChar(100))
            .execute("USP_UserLoginActivityLog");
        return result.output.ResultMessage;
    } catch (error) {
