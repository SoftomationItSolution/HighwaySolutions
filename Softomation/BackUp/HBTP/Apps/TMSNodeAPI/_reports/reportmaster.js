...
function getDateFromDateTime(dateString){
    ...
    return date.toLocaleDateString('en-GB', options).replace(',', '').split(' ').join('-');
}

function generateCashUpPdf(GeneratedBy, headerDetail, ReportDataList, pdfName) {
    try {
        const ProjectConfiguration = require(pc_Directory);
        const ReportName = headerDetail.ReportType
        const currentPath = path.resolve('./images');
                ...
                if (currentPage > 1) {
                    return {
                        columns: [
                            {
                                width: '*',
                                stack: [{ text: ReportName + ' Report', alignment: 'left', fontSize: 8, bold: true }]
                            },
                            {
                                width: '*',
                                text: '',
                ...
                ...ReportDataList.map((dataSet, index) => {
                    if (index > 0) {
                        if (dataSet.length > 0) {
                            ...
                            return [
                                { text: `Shift : ${dataSet[0]["Shift"]}  Date: ${getDateFromDateTime(headerDetail.StartDateTime)}`, alignment: 'left', bold: true, margin: [0, 10] },
                                {
