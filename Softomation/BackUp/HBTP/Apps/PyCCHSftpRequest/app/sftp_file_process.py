...
class SftpProcessThread(threading.Thread):
    ...
    def run(self):
        try:
            ...
            last_downloaded_timestamp=self.get_last_status()
            while self.flag:
                try:
                    if self.sftp is None:
                        ...
                        if remote_files:
                            for file in remote_files:
                                ...
                                if file_date_time:
                                    if last_downloaded_timestamp is None or filetimestamp > last_downloaded_timestamp:
                                        ...
                                        last_downloaded_timestamp = filetimestamp
                                    ...
                                    self.logger.logError(f"file time stamp is missing: {file.filename}")
                except Exception as e:
