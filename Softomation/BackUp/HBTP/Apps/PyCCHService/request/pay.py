...
class RequestPayAPI(threading.Thread):
    ...
    def create_lane(self,root,row):
        try:
            ...
            lane.set("laneType", str(row["LaneTypeName"]).replace("Booth Less","Hybrid"))
            lane.set("ExitGate", "")
            lane.set("Floor", "")
            return lane
        except Exception as e:
    ...
    def create_entry_lane(self,root,row):
        try:
            ...
            entry_lane.set("laneType", str(row["LaneTypeName"]).replace("Booth Less","Hybrid"))
            entry_lane.set("EntryGate", "")
    ...
    def generate_xml(self, row):
        try:
            ...
            if signed_xml:    
                try:
                    response=SendRequest.upload_request(signed_xml, F"{self.icd_api_detail['BaseUrl']}{self.icd_api_detail['RequestPayURL']}",certificate_validation=1)
                    if response.ok:
